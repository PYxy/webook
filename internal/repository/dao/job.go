...
func (g *GORMJobDAO) UpdateUtime(ctx context.Context, id int64, version int) error {
	return g.db.WithContext(ctx).Model(&Job{}).
		Where("id =? AND version =?", id, version).Updates(map[string]any{
		"utime": time.Now().UnixMilli(),
	}).Error
}

func (g *GORMJobDAO) UpdateNextTime(ctx context.Context, id int64, next time.Time, version int) error {
	return g.db.WithContext(ctx).Model(&Job{}).
		Where("id = ? AND version =?", id, version).Updates(map[string]any{
...
func (g *GORMJobDAO) Stop(ctx context.Context, id int64, version int) error {
...
func (g *GORMJobDAO) Release(ctx context.Context, id int64, version int) error {
	...
	// WHERE version = ?
	// 要。你们的作业记得修改
	return g.db.WithContext(ctx).Model(&Job{}).Where("id =? AND verison =?", id, version).
...
func (g *GORMJobDAO) Preempt(ctx context.Context) (Job, error) {
	...
	for {
		...
		err := db.WithContext(ctx).Where("status = ? AND next_time <=?", jobStatusWaiting, now.UnixMilli()).Or("status = ? AND utime < ?", jobStatusRunning, now.Add(-time.Minute*time.Duration(3)).UnixMilli()).
			First(&j).Error
		// 你找到了，可以被抢占的
		// 找到之后你要干嘛？你要抢占
		if err != nil {
		...
		// 能不能用 utime? 不能 1毫秒就可以同时有多个协程运行了
